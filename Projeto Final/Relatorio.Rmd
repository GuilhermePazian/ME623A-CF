...
setwd(choose.dir())
dados <- read.csv2('testedados.csv',header = TRUE)
...
Periodo <- ggplot(dados,aes(Periodo,Tempo_m)) + geom_boxplot()


Pessoa <- ggplot(dados,aes(Pessoa,Tempo_m)) + geom_boxplot()


Dia <- ggplot(dados,aes(Dia,Tempo_m)) + geom_boxplot()
...
```{r echo=FALSE}
#modelo e ANOVA
fit <- lm(Tempo_m~ Periodo + Dia + Pessoa, data = dados)
tblanova <- anova(fit)
